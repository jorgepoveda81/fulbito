...
function renderHomeScreen(container, onPlay){
  ...
  const user = status === 'ready' ? getCurrentUser() : null;

  container.innerHTML = `
    <div class="home-card">
      ${status === 'loading' ? `<p class="creator-hint">Conectando tu cuenta...</p>` : ''}
      ${status === 'failed' ? `<p class="creator-hint">&#9888; ${error}</p>` : ''}
      ${user ? `
      ...
      ` : ''}
      ${status === 'unconfigured' ? `<p class="creator-hint">Jugando en modo local: Mi Equipo y la Tienda se guardan del todo cuando se configure Firebase (ver README).</p>` : ''}
    </div>
