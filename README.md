...
js/online-repo.js               salas de partido online por codigo (crear/unirse/escuchar en Firestore)
js/online-ui.js                  pantalla de conexion del modo online (crear/unirse a una sala)
...
- Modo online por turnos: "Jugar online" desde Inicio, sala por codigo de 5 caracteres, cada jugador arma su equipo en su propio celular y despues juega su turno cuando puede. Si el rival cierra la sala o se sale (antes o durante el partido), se avisa y se vuelve solo al menu en vez de dejarte mirando una partida congelada. Probado de punta a punta con una simulacion (dos celulares "de mentira" en vez de Firestore real, ver "Proximos pasos sugeridos") que encontro y corrigio varias fallas que lo rompian del todo — la mas seria: cuando una jugada le pasaba el turno al OTRO equipo (una recuperacion, un gol, la tanda de penales), el aviso al rival se perdia y la partida quedaba congelada; pasaba en casi cualquier jugada normal, no solo en casos raros. Ya se verifico un turno completo de verdad (tiro real con arrastre) yendo y viniendo entre los dos lados, un cambio de posesion real sincronizandose bien, y que el celular que espera no se "robe" el turno solo por su cuenta. Falta la prueba con dos celulares y cuentas reales.
...
- La IA sigue siendo pasa-hacia-adelante-y-tira-cerca-del-arco (no jugadas elaboradas ni cuenta propia), pero ahora la dificultad cambia que tan bien lo hace: en Facil reacciona lento, apunta con ruido, tira solo de muy cerca y calibra mal la fuerza; en Dificil reacciona casi al toque, apunta al lado del arco mas lejos del arquero rival, se anima a tirar desde mas lejos y calibra la fuerza con consistencia. En Dificil ademas juega sus poderes con criterio: Impulso antes de tirar, Silencio para dejar al rival sin poderes si recupera el balon, Escudo de aura como preparacion temprana, Farmear aura tras 2 pases seguidos, Tiempo extra si la carga no entra en el reloj, y Segundo bloqueo si su primer intento de intercepcion fallo mientras el rival tira. Nunca ve mas de lo que veria un jugador humano (todo el estado es siempre visible para los dos lados, y los poderes elegidos siguen siendo secretos hasta que se usan).
...
- Las monedas se suman desde el navegador al terminar el partido (con `increment()` de Firestore, asi que dos escrituras que se solapan no se pisan entre si). `firestore.rules` limita cuanto puede subir de una sola vez (coins hasta 50, wins/losses/draws hasta 1, los maximos reales de un solo partido), asi que llamar a esa funcion a mano desde la consola ya no sirve para hacerse rico de un tiro; alguien insistente igual podria llamarla muchas veces seguidas para juntar de a poco — evitar eso del todo ya necesita una funcion de servidor (Firebase Cloud Functions). Para una cuenta privada entre ustedes dos no es un problema real.
...
4. **Modo online (fase 1 ya hecha)**: desde Inicio, "Jugar online" crea o une una sala por codigo de 5 caracteres. Cada jugador elige sus poderes y arma su formacion en su propio celular (el rival no ve nada hasta que arranca el partido), y despues se juega por turnos: cada quien juega su turno cuando puede, el estado se guarda en Firestore y el otro celular lo recibe la proxima vez que abre la app — si los dos tienen la app abierta a la vez, los turnos igual les llegan casi al instante porque Firestore avisa en vivo, aunque el balon en pleno vuelo no se ve animado en el otro celular (eso queda para una fase 2). Si el anfitrion cierra la sala o el invitado se sale (antes o durante el partido), el que queda ve un aviso y vuelve solo al menu. Este entorno de desarrollo no tiene acceso a Firebase real, asi que no se pudo probar contra Firestore de verdad; en cambio se simulo el intercambio completo con dos celulares "de mentira" conectados por un relay armado a mano en vez de Firestore (crear la sala, cada uno arma su equipo en privado, arranca el partido, sorteo incluido, jugar turnos de verdad con arrastre real). Asi se encontraron y corrigieron varias fallas reales que rompian el modo online de punta a punta: la pantalla de formacion online quedaba tapada por un menu que nunca se ocultaba (nadie podia tocar nada); si el invitado ganaba el sorteo la partida quedaba congelada para siempre de su lado; y la mas grave — cada vez que una jugada le pasaba el turno al OTRO equipo (una recuperacion, un gol, la tanda de penales, no solo el sorteo) el aviso al rival se perdia porque el chequeo de "es mi turno" bloqueaba justo esa publicacion, dejando la partida congelada en casi cualquier jugada normal, no solo en casos raros. De paso salio otra: el celular que esta esperando tenia su propio reloj de turno corriendo sin control y podia "robarse" el turno el solo despues de unos segundos, sin que el rival hiciera nada. Con la simulacion ya se probo un turno completo de verdad (arrastre real sobre la cancha, tiro, rebotes, cambio de posesion real, y el otro celular quedando exactamente igual al terminar), pero falta la prueba real con dos celulares y cuentas de verdad antes de confiar en ella del todo (latencia real, reglas de Firestore, cortes de conexion de verdad).
